...
if (!class_exists('Gpf_Rpc_Server', false)) {
  class Gpf_Rpc_Server extends Gpf_Object {
      ...
      public function execute($request = '') {
          try {
              $requestData = Gpf_Http::getRequestVariable(self::BODY_DATA_NAME);
              if ($requestData != null) {
                  $request = $requestData;
              }
          ...
          if ($this->isFormRequest($request) && $response instanceof Gpf_Rpc_Form && $params->get(self::FORM_RESPONSE) != self::HANDLER_WINDOW_NAME) {
      ...
      private function setDecoder($request) {
          if ($this->isFormRequest($request)) {
              Gpf_Http::setHeader('Content-Type', 'text/plain; charset=utf-8');
              ...
              Gpf_Http::setHeader('Content-Type', 'text/plain; charset=utf-8');
              Gpf_Http::setHeader('HTTP/1.1 400 Bad Request', null, 400);
              die('Invalid request');
          }
      }
      ...
      public function syncTime(Gpf_Rpc_Params $params) {
          ...
          if (abs($timeOffset) > (24 * 60 * 60)) {
              $timeOffset = $timeOffset % (24 * 60 * 60);
          }
          Gpf_Session::getInstance()->setTimeOffset($timeOffset);
          $action->addOk();
          return $action;
      }
  }
  ...
  class Gpf_Rpc_MultiRequest extends Gpf_Object {
      ...
      protected function sendRequest($requestBody) {
          ...
          if ($response->getResponseCode() == 429) {
              throw new Gpf_Exception($this->_('Too Many Requests. Please try to send request later.'));
          }
       ...
       * Should be used only in API
       * @throws Gpf_Exception
       * @throws Gpf_Rpc_ExecutionException
       */
...
if (!class_exists('Gpf_Rpc_Params', false)) {
  class Gpf_Rpc_Params extends Gpf_Object implements Gpf_Rpc_Serializable {
      ...
      public function getSessionId() {
          ...
          if (is_array($sessionId) || is_object($sessionId)) {
              $sessionId = null;
          }
...
if (!interface_exists('Gpf_HttpResponse', false)) {
  interface Gpf_HttpResponse {
      ...
      public function setHeaderValue($name, $value, $replace = true, $httpResponseCode = 0);
  }
...
if (!class_exists('Gpf_Http', false)) {
  class Gpf_Http extends Gpf_Object implements Gpf_HttpResponse {
      ...
      private static ?Gpf_Http $instance = null;
      ...
      public static function setInstance(Gpf_Http $instance = null): void {
          self::$instance = $instance;
      }
  
      public static function getRequestVariable($name) {
          if (!is_array($_REQUEST) || !array_key_exists($name, $_REQUEST)) {
              return null;
          }
      ...
      public static function setSecuredCookie($name, $value = null, $expire = null, $path = null, $domain = null) {
          self::getInstance()->setCookieValue($name, $value, $expire, $path, $domain, true, true);
      }
  
      public static function setHeader($name, $value, $httpResponseCode = 0) {
      ...
      public function setCookieValue($name, $value = null, $expire = null, $path = null, $domain = null, $secure = null, $httpOnly = null) {
          $cookieExpire = '';
          if ($expire != null) {
              $cookieExpire = '; Expires='.gmdate('D, d M Y H:i:s', $expire) . ' GMT';
          }
          $cookiePath = '';
          if ($path != null) {
          ...
          header('Set-Cookie: ' . rawurlencode($name) . '=' . rawurlencode($value) . $cookieExpire . $cookiePath . $cookieDomain . $cookieSecure. $cookieHttpOnly . $sameSite, false);
      }
      ...
      public static function getUserAgent() {
          $userAgent = self::getUserAgentPart();
      ...
      public static function getUserAgentPart() {
      ...
      /**
       * @param string $userAgent = $_SERVER['HTTP_USER_AGENT'].' - '.$_SERVER['HTTP_ACCEPT_LANGUAGE']
       */
      public static function setUserAgent(string $userAgent): void {
          $userAgentArray = explode(' - ', $userAgent);
          if (count($userAgentArray) == 2) {
              $_SERVER['HTTP_USER_AGENT'] = $userAgentArray[0];
              $_SERVER['HTTP_ACCEPT_LANGUAGE'] = $userAgentArray[1];
          } else {
              $_SERVER['HTTP_USER_AGENT'] = $userAgent;
          }
      }
  
      public static function getRemoteIp($onlyRemote = false) {
          return self::anonymizeIP(self::getRemoteIpFull($onlyRemote));
      }
  
      public static function getRemoteIpFull($onlyRemote = false) {
          if (isset($GLOBALS['TRACKING_IP_ADDRESS']) && $GLOBALS['TRACKING_IP_ADDRESS'] != '') {
              return $GLOBALS['TRACKING_IP_ADDRESS'];
          }
          if (isset($_SERVER['HTTP_X_FORWARDED_FOR'])) {
              $ip = self::parseXForwardedForIp($_SERVER['HTTP_X_FORWARDED_FOR']);
              if ($ip != '') {
                  return $ip;
              }
          }
      ...
      public static function setIpAddress(string $ipAddress): void {
          $GLOBALS['TRACKING_IP_ADDRESS'] = $ipAddress;
      }
  
      public static function parseXForwardedForIp($ip) {
          if (!strstr($ip, ',')) {
      ...
      public static function setStrictTransportSecurityHeader() {
          self::setHeader('Strict-Transport-Security', 'max-age=10886400; includeSubDomains');
      }
...
if (!class_exists('Gpf_Data_Record', false)) {
  class Gpf_Data_Record extends Gpf_Object implements Iterator, Gpf_Rpc_Serializable,
      ...
      public function translateRowColumn($column): void {
          if ($this->get($column) === '' || $this->get($column) === null) {
              $this->set($column, '');
          } else {
              $this->set($column, $this->_localizeNoReplace($this->get($column)));
          }
      }
  }
...
if (!class_exists('Gpf_Data_Grid', false)) {
  class Gpf_Data_Grid extends Gpf_Object {
      ...
      public function loadFromObject(stdClass $object) {
          ...
          $this->lastValue = $object->last_value;
          $this->lastId = $object->last_id;
      }
  ...
  class Gpf_Rpc_GridRequest extends Gpf_Rpc_Request {
      ...
      public function send() {
          ...
          try {
              $grid = $this->getGrid();
              if ($grid->getLastId() != '') {
                  $this->addParam('last_id', $grid->getLastId());
              }
              if ($grid->getLastValue() != '') {
                  $this->addParam('last_value', $grid->getLastValue());
              }
              if ($grid->getTotalCount() != '') {
                  $this->addParam('rows_count', $grid->getTotalCount());
              }
...
if (!class_exists('Gpf_Data_RecordSet', false)) {
  class Gpf_Data_RecordSet extends Gpf_Object implements IteratorAggregate, Gpf_Rpc_Serializable {
      ...
      public function loadFromObject($array) {
          if($array === null) {
              throw new Gpf_Exception('Array cannot be NULL');
          }
...
if (!class_exists('Gpf_Net_Http_Request', false)) {
  class Gpf_Net_Http_Request extends Gpf_Object {
  	const CRLF = "\r\n";
      const NO_SSL_VERIFYHOST = 0;
      const SSL_VERIFYHOST_EASY = 1;
      const SSL_VERIFYHOST_NORMAL = 2;
  	...
  	private $fragment = '';
  	private $cookies = '';
      private $sslVerifyHost = self::SSL_VERIFYHOST_NORMAL;
  	private $maxTimeout = null;
  	private $userAgent = '';
  	...
  	public function addQueryParam($name, $value): void {
  		if (is_array($value)) {
  	...
  	private function initHeaders() {
  		...
  		if ($this->getMethod() == 'POST' && $this->getHeader("Content-Type") == '') {
          ...
          $body = (is_array($this->getBody())) ? http_build_query($this->getBody()) : $this->getBody();
...
if (!class_exists('Gpf_Net_Http_ClientBase', false)) {
  abstract class Gpf_Net_Http_ClientBase extends Gpf_Object {
      ...
      public function execute(Gpf_Net_Http_Request $request, Gpf_Net_Http_SafeUrl_Options $options = null) {
          ...
          $request->setFollowLocation(false); //security restriction (e.g. local port scanning attack)
          if ($options !== null) {
              Gpf_Net_Http_SafeUrl_Url::validateUrl($request->getUrl(), $options);
          }
  
          $this->setProxyServer($request);
          if (Gpf_Php::isFunctionEnabled('curl_init') && Gpf_Php::isFunctionEnabled('curl_exec')) {
              $response = $this->executeWithCurl($request);
          } else {
      ...
      private function executeWithCurl(Gpf_Net_Http_Request $request) {
          ...
          @curl_setopt($session, CURLOPT_MAXREDIRS, self::MAX_REDIRECTS);
          @curl_setopt($session, CURLOPT_RETURNTRANSFER, true);
          if ($request->getFailOnError()) {
          ...
          @curl_setopt($session, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4);
          if ($request->getMaxTimeout() != '') {
...
if (!class_exists('Gpf_Rpc_Form', false)) {
  class Gpf_Rpc_Form extends Gpf_Object implements Gpf_Rpc_Serializable, IteratorAggregate {
      ...
      public function getFieldValue($name) {
          $record = $this->fields->getRecord($name);
          $value = $record->get(self::FIELD_VALUE);
  
          if ($value == null || is_scalar($value) || $value instanceof Gpf_Rpc_Serializable) {
              return $value;
          }
          throw new Gpf_Exception('Not supported value for ' . $name);
      }
  
      /**
       * @param $name
       * @return array
       * @throws Gpf_Data_RecordSetNoRowException
       * @throws Gpf_Exception
       */
      public function getFieldValueArray($name): array {
          $record = $this->fields->getRecord($name);
          $value = $record->get(self::FIELD_VALUE);
      ...
      public function load(Gpf_DbEngine_Row|Gpf_DbEngine_RowComposite $row): void {
      ...
      public function fill(Gpf_Data_Row $row, array $writableFields = [], array $allowedFieldsValues = []) {
          foreach ($this->fields as $field) {
              $fieldName = $field->get(self::FIELD_NAME);
              if (count($writableFields) > 0 && !in_array($fieldName, $writableFields)) {
                  continue;
              }
              try {
                  $fieldValue = $field->get(self::FIELD_VALUE);
                  if (is_string($fieldValue)) {
                      $fieldValue = trim($fieldValue);
                  }
                  if (count($allowedFieldsValues) > 0 && array_key_exists($fieldName, $allowedFieldsValues)) {
                      $allowedFieldValues = $allowedFieldsValues[$fieldName];
                      if (!in_array($fieldValue, $allowedFieldValues)) {
                          continue;
                      }
                  }
      ...
      public function getFieldErrorMessage() {
          foreach ($this->getIterator() as $field) {
              if ($field->get(self::FIELD_ERROR) != '') {
                  return $field->get(self::FIELD_ERROR);
              }
          }
          return '';
      }
  ...
  class Gpf_Rpc_Form_Validator_FormValidatorCollection extends Gpf_Object {
      ...
      public function validate() {
          ...
          foreach ($this->validators as $fieldName => $fieldValidator) {
              try {
                  $value = $this->form->getFieldValue($fieldName);
          ...
          if ($errorMsg) {
              $this->form->setErrorMessage($this->_('There were errors, please check the highlighted fields'));
          }
          return !$errorMsg;
      }
  }
...
if (!class_exists('Gpf_Rpc_DataRequest', false)) {
  class Gpf_Rpc_DataRequest extends Gpf_Rpc_Request {
       ...
       * adds filter to grid
       */
      public function addFilter(string $code, string $operator, $value): void {
          $this->filters[] = new Gpf_Data_Filter($code, $operator, $value);
      }
...
if (!class_exists('Gpf_Rpc_PhpErrorHandler', false)) {
  class Gpf_Rpc_PhpErrorHandler {
      ...
      public function callMethod($callback, $params = null) {
...
if (!class_exists('Gpf_Rpc_Action', false)) {
  class Gpf_Rpc_Action extends Gpf_Object implements Gpf_Rpc_Serializable {
      ...
      /**
       * @return ArrayIterator|Gpf_View_GridService_IdsIterator
       * @throws Gpf_Exception
       */
      public function getIds() {
      ...
      protected function getRequestIdsIterator() {
          ...
          $requestParams->set('columns', array());
...
if (!class_exists('Gpf_Log', false)) {
  class Gpf_Log  {
       ...
       *
       * @param string|Gpf_Log_LoggerBase $type
      ...
      /**
       * @param $type
       * @return Gpf_Log_LoggerBase|null
       */
      public static function removeLogger($type) {
          return self::getLogger()->removeLogger($type);
      }
...
if (!class_exists('Gpf_Log_Logger', false)) {
  class Gpf_Log_Logger extends Gpf_Object {
       ...
       */
      public function checkLoggerTypeExists($type): bool {
       ...
       * @return Gpf_Log_LoggerBase|null
       */
      public function removeLogger($type) {
          $logger = null;
          if (array_key_exists($type, $this->loggers)) {
              $logger = $this->loggers[$type];
      ...
      public function log($message, $logLevel, $logGroup = null) {
          ...
          if (isset($callingFile['classVariables']) && $callingFile['classVariables'] != '') {
              $file .= ' '.$callingFile['classVariables'];
          }
          $line = $callingFile['line'];
  
          foreach ($this->loggers as $logger) {
              if ($logger->getType() == Gpf_Log_LoggerDatabase::TYPE) {
                  $ip = Gpf_Http::getRemoteIp();
              } else {
                  $ip = Gpf_Http::getRemoteIpFull();
              }
              if ($ip == '') {
                  $ip = '127.0.0.1';
              }
      ...
      private function sqlBenchmarkFound($calls): bool {
          foreach($calls as $obj) {
              if (isset($obj['function']) && $obj['function'] == 'endBenchmark') {
                  return true;
              }
          }
          return false;
      ...
      private function findFileBySqlBenchmark() {
          $foundFile = $this->findLatestObjectBeforeStrings('DbEngine', 'SqlBuilder');
      ...
      private function findCallingFile($calls, $functionName) {
          ...
          foreach($calls as $obj) {
              if (isset($obj['function']) && $obj['function'] == $functionName) {
                  $previousObject = $obj;
                  if (isset($obj['args'][0])) {
                      $logMessage = $obj['args'][0];
                  }
              } elseif ($logMessage != null) {
                  if (isset($obj['args'][0]) && is_string($obj['args'][0]) && $logMessage == $obj['args'][0]) {
                      $previousObject = $obj;
                  } else {
      ...
      private function findLatestObjectBeforeStrings(string $text1, string $text2) {
          $callsReversed = array_reverse(debug_backtrace());
          ...
          foreach($callsReversed as $obj) {
              ...
              if ($lastObject != null) {
                  $pos = strpos($obj['file'], $text1);
                  $pos2 = strpos($obj['file'], $text2);
...
if (!class_exists('Gpf_Api_IncompatibleVersionException', false)) {
  class Gpf_Api_IncompatibleVersionException extends Exception {
      ...
      public function __construct($url) {
          $this->apiLink = str_replace('scripts/server.php', '', $url). 'merchants/#Api-Integration';
          parent::__construct('Version of API does not correspond to the Application version. Please <a href="' . $this->apiLink . '" target="_blank">download the latest version of API</a>.', 0);
      }
...
if (!class_exists('Gpf_Api_Session', false)) {
  class Gpf_Api_Session extends Gpf_Object {
  	 ...
  	 * @param string $languageCode language code (e.g. en-US, de-DE, sk, cz, du, ...)
  	 * @return boolean true if user was successfully logged
  	 */
      public function login($username, $password, $roleType = self::MERCHANT, $languageCode = null, $twofactorToken = null) {
      	return $this->authenticateRequest($username, $password, '', $roleType, $languageCode, $twofactorToken);
      }
       ...
       * @return boolean true if user was successfully logged
       */
      public function loginWithLoginKey($loginKey, $roleType = self::MERCHANT, $languageCode = null) {
          return $this->authenticateRequest('', '', '', $roleType, $languageCode, '', $loginKey);
      }
  	 ...
  	 * @return boolean true if user was successfully logged
  	 */
      private function authenticateRequest($username, $password, $authtoken, $roleType = self::MERCHANT, $languageCode = null, $twofactorToken = null, $loginKey = null) {
          ...
          if ($username != '' && $password != '') {
              ...
              $request->setField('LoginKey', $loginKey);
          } else {
              $request->setField('authToken', $authtoken);
          }
          if ($twofactorToken != '') {
              $request->setField('twofactor_token', $twofactorToken);
          }
       ...
       *
       * @return string|false version of installed application
       */
      public function getAppVersion() {
          $request = new Gpf_Rpc_FormRequest($this->getAuthenticateClassName(), "getAppVersion", $this);
          $request->setUrl($this->url);
      ...
      public static function getAPIVersion(): string {
          if (defined('PAP_API_VERSION')) {
              return PAP_API_VERSION;
          }
...
if (!class_exists('Gpf_Rpc_Json', false)) {
  class Gpf_Rpc_Json implements Gpf_Rpc_DataEncoder, Gpf_Rpc_DataDecoder {
       ...
       *
       * @param array|string $str JSON-formatted string
       *
      ...
      public function decode($str, $assoc = false) {
          ...
          if ($assoc) {
              $this->use = self::SERVICES_JSON_LOOSE_TYPE;
          }
      ...
      public static function decodeStatic($var, $assoc = false) {
          return self::getInstance()->decode($var, $assoc);
      }
...
if (!class_exists('Pap_Api_Object', false)) {
  class Pap_Api_Object extends Gpf_Object {
      ...
      protected function getPrimaryKeyFromFields(): void {
...
if (!class_exists('Pap_Api_Affiliate', false)) {
  class Pap_Api_Affiliate extends Pap_Api_Object {
      ...
      protected function getDataFields() {
          ...
          $request->addParam('includeCustoms', Gpf::YES);
      ...
      protected function getGridRequest() {
          $gridRequest = new Pap_Api_AffiliatesGridSimple($this->getSession());
          $gridRequest->addParam('columns', new Gpf_Rpc_Array(array(array('id'))));
          return $gridRequest;
      }
  
      protected function getDataFieldStatuses() {
          return 'M,O,P,R,S,W,H';
      }
      ...
      private function sendActionRequest($className, $method, Gpf_Rpc_Params $params) {
          ...
          $request->sendNow();
          return $request->getResponseObject()->toObject();
      }
      ...
      public function getLoginKey() {
          ...
          $request = new Gpf_Rpc_FormRequest('Pap_Auth_LoginKeyService', 'getLoginKey', $this->getSession());
          if ($this->getUserid() != '') {
              $request->addParam('userId', $this->getUserid());
          } else if ($this->getUsername() != '') {
              $request->addParam('userName', $this->getUsername());
          } else {
              throw new Gpf_Exception('User Id or Username is required');
          }
          try {
              $request->sendNow();
          } catch (Gpf_Exception $e) {
          ...
          $form = $request->getForm();
          if ($form->isError()) {
              throw new Gpf_Exception($form->getErrorMessage());
          }
  
          return $request->getForm()->getFieldValue('LoginKey');
      }
  }
...
if (!class_exists('Pap_Api_Transaction', false)) {
  class Pap_Api_Transaction extends Pap_Api_Object {
      ...
      public function getSplit() { return $this->getField("split"); }
      public function setSplit($value) { $this->setField("split", $value); }
       ...
       * @param int $refundValue value for partial refund
       * @return Gpf_Rpc_Action
       */
      public function chargeBack($note = '', $fee = 0, $refundMultiTier = false, $refundValue = 0) {
          return $this->makeRefundChargeBack($note, 'H', $fee, $refundMultiTier, $refundValue);
      }
       ...
       * @return Gpf_Rpc_Action
       */
      public function refund($note = '', $fee = 0, $refundMultiTier = false, $refundValue = 0) {
          return $this->makeRefundChargeBack($note, 'R', $fee, $refundMultiTier, $refundValue);
      }
       ...
       * @return Gpf_Rpc_Action
       */
      public function chargeBackByOrderId($note = '', $fee = 0, $refundValue = 0) {
          return $this->makeRefundChargeBackByOrderId($note, 'H', $fee, $refundValue);
      }
       ...
       * @return Gpf_Rpc_Action
       */
      public function refundByOrderId($note = '', $fee = 0, $refundValue = 0) {
          return $this->makeRefundChargeBackByOrderId($note, 'R', $fee, $refundValue);
      }
  
      /**
       * @return Gpf_Rpc_Action
       */
      private function makeRefundChargeBack($note, $type, $fee, $refundMultiTier, $refundValue) {
          if ($this->getTransid() == '') {
       ...
       * @return Gpf_Rpc_Action
       */
      private function makeRefundChargeBackByOrderId($note, $type, $fee, $refundValue) {
          if ($this->getOrderId() == '') {
              throw new Gpf_Exception("Order Id is empty. Call setOrderId() or load transaction before calling refund/chargeback");
          }
...
if (!class_exists('Pap_Tracking_Action_RequestActionObject', false)) {
  class Pap_Tracking_Action_RequestActionObject extends Gpf_Rpc_JsonObject {
      ...
      public $dndc = ''; // do not delete cookies
      public $chc  = ''; // changed commission
      ...
      public function getChangedCommission() {
          return $this->chc;
      }
  
      public function isDoNotDeleteCookies() {
          return $this->dndc == Gpf::YES;
      }
      ...
      public function setChangedCommission($value) {
          $this->chc = $value;
      }
  
      public function doNotDeleteCookies() {
          $this->dndc = Gpf::YES;
      }
...
if (!class_exists('Pap_Tracking_Request', false)) {
  class Pap_Tracking_Request extends Gpf_Object {
      ...
      public function getUser(): ?Pap_Affiliates_User {
          try {
      ...
      public function getRawExtraData(int $i): string {
          $extraDataParamName = $this->getSaleExtraDataParamName($i);
      ...
      public function getRequestParameter($paramName, $returnString = true) {
      ...
      public static function getRotatorBannerParamName() {
          return Gpf_Settings::get(Pap_Settings::PARAM_NAME_ROTATOR_ID);
      }
  
      public static function getSpecialDestinationUrlParamName() {
      ...
      public function getSaleExtraDataParamName($i) {
          return Gpf_Settings::get(Pap_Settings::PARAM_NAME_EXTRA_DATA).$i;
      }
  
      public static function getExtraDataParamName($i) {
          return Gpf_Settings::get(Pap_Settings::PARAM_NAME_EXTRA_DATA.$i);
      }
      ...
      public function toString() {
          ...
          foreach($this->request as $key => $value) {
              $params .= ($params != '' ? ", " : '').Gpf_Common_String::convertToUtf8($key).'='.Gpf_Common_String::convertToUtf8(print_r($value, true));
          }
          return $params;
      }
      ...
      public static function getAffiliateClickParamName() {
          return Gpf_Settings::get(Pap_Settings::PARAM_NAME_USER_ID);
      }
  
      public static function getBannerClickParamName() {
      ...
      public static function getChannelParamName(): string {
      ...
      public static function getForcedAffiliateParamName(): string {
          return Pap_Tracking_Request::PARAM_FORCED_AFFILIATE_ID;
      }
  
      public static function getForcedBannerParamName(): string {
      ...
      public static function getForcedCampaignParamName(): string {
       ...
       */
      public static function encodeRefererUrl(?string $url): string {
          return str_replace(['http://', 'https://'], ['H_', 'S_'], $url);
      }
  
      /**
       * This function does decoded encoded url
       */
      public static function decodeRefererUrl($url): ?string {
          if (str_starts_with($url ?? '', 'H_')) {
              return 'http://' . substr($url, 2);
          }
          if (str_starts_with($url ?? '', 'S_')) {
...
if (!class_exists('Pap_Api_Tracker', false)) {
  class Pap_Api_Tracker extends Gpf_Object {
      ...
      private $ip;
      private $userAgent;
      private $paramNameUserId = 'a_aid';
      private $overwriteCookie = false;
      /**
       * @var array<Pap_Tracking_Action_RequestActionObject>
       */
      private $sales = array();
      const VISITOR_COOKIE_NAME = 'PAPVisitorId';
      const AFFILIATE_COOKIE_NAME = 'PAPAffiliateId';
  
      const NOT_LOADED_YET = '-1';
      private Gpf_Rpc_Data|string|null $affiliate = self::NOT_LOADED_YET;
      private Gpf_Rpc_Data|string|null $campaign = self::NOT_LOADED_YET;
      private Gpf_Rpc_Data|string|null $channel = self::NOT_LOADED_YET;
      ...
      public function __construct(Gpf_Api_Session $session) {
          ...
          $this->session = $session;
          $this->setVisitorId($this->getVisitorIdFromCookie());
      }
      ...
      public function setVisitorId($visitorId) {
          $this->visitorId = $this->trimQuotationMarks($visitorId);
      }
      ...
      public function setParamNameUserId($paramNameUserId) {
          $this->paramNameUserId = $paramNameUserId;
      }
  
      public function setOverwriteCookie($overwriteCookie = true) {
          $this->overwriteCookie = $overwriteCookie;
      }
  
      public function track() {
          if (count($this->sales) == 0) {
              $this->trackClickRequest();
              return;
          }
  
          foreach ($this->sales as $sale) {
              $request = $this->createTrackRequest();
              $saleParams = Gpf_Rpc_Json::encodeStatic(array($sale));
              $request->addQueryParam('sale', $saleParams);
              ...
              $this->executeTrackRequest($request);
          }
      }
       ...
       */
      private function createTrackRequest() {
          $request = new Gpf_Net_Http_Request();
      ...
      private function executeTrackRequest(Gpf_Net_Http_Request $request) {
      ...
      private function trackClickRequest() {
          $request = $this->createTrackRequest();
      ...
      public function save3rdPartyCookiesOnly($cookieDomainValidity = null) {
          ...
          $this->save3rdPartyCookie(self::VISITOR_COOKIE_NAME, $this->visitorId, time() + 31536000, true, $cookieDomainValidity);
          $this->saveAffiliateCookie($cookieDomainValidity);
      }
  
      private function saveAffiliateCookie($cookieDomainValidity = null) {
          if (isset($_GET[$this->paramNameUserId]) && $_GET[$this->paramNameUserId] != '') {
              if ($this->overwriteCookie || !$this->existsAffiliateCookie()) {
                  $this->save3rdPartyCookie(self::AFFILIATE_COOKIE_NAME, $_GET[$this->paramNameUserId], time() + 300, true, $cookieDomainValidity);
              }
          }
      }
      ...
      private function getData(&$data, $method, $primaryKeyName) {
          ...
          if ($data === self::NOT_LOADED_YET) {
              ...
              if (isset($_GET[$this->paramNameUserId]) && $_GET[$this->paramNameUserId] != '') {
                  $request->addParam('userId', $_GET[$this->paramNameUserId]);
              } else if ($this->existsAffiliateCookie()) {
                  $request->addParam('userId', $_COOKIE[self::AFFILIATE_COOKIE_NAME]);
              } else if ($this->getTrackerAffiliateId() != '') {
                  $request->addParam('userId', $this->getTrackerAffiliateId());
              }
      ...
      private function existsAffiliateCookie() {
          return isset($_COOKIE[self::AFFILIATE_COOKIE_NAME]) && !is_array($_COOKIE[self::AFFILIATE_COOKIE_NAME]) && $_COOKIE[self::AFFILIATE_COOKIE_NAME] != '';
      }
  
      protected function getTrackerAffiliateId() {
          return '';
      }
      ...
      protected function getUrl() {
          ...
          return 'https://' . $this->getServerName() . $portString . $scriptName;
      }
      ...
      private function trimQuotationMarks($str): ?string {
          if($str != null && (str_contains($str, '"') || str_contains($str, "'"))) {
              $str = trim($str, '"');
              return trim($str, "'");
          }
          return $str;
...
if (!class_exists('Pap_Api_RecurringCommission', false)) {
  class Pap_Api_RecurringCommission extends Pap_Api_Object {
      ...
      public function setData1($value) {
          $this->setField('data1', $value);
      }
  
      public function setData2($value) {
          $this->setField('data2', $value);
      }
  
      public function setData3($value) {
          $this->setField('data3', $value);
      }
  
      public function setData4($value) {
          $this->setField('data4', $value);
      }
  
      public function setData5($value) {
          $this->setField('data5', $value);
      }
      ...
      public function createCommissions() {
          $request = new Gpf_Rpc_ActionRequest($this->class, 'createCommissions', $this->getSession());
          $this->addRecurringCommissionParams($request);
          $request->sendNow();
      ...
      public function createCommissionsReturnIds() {
          $request = new Gpf_Rpc_DataRequest($this->class, 'createCommissionsReturnIds', $this->getSession());
          $this->addRecurringCommissionParams($request);
          $request->sendNow();
      ...
      private function addRecurringCommissionParams(Gpf_Rpc_Request $request) {
          ...
          for ($i = 1; $i <= 5; $i++) {
              if ($this->getField('data' . $i)) {
                  $request->addParam('data' . $i, $this->getField('data' . $i));
              }
          }
